...
alias adg='emerge --deep --newuse --getbinpkg=y -vt --update @world'
...
alias f='cd /media/ntfsFILES/ ; clear_console'
...
alias h='cd /home/k4ndar3c/ ; clear_console'
alias h='cd /home/k4ndar3c/MPoint ; clear'
...
alias Kitploit='python2 /home/k4ndar3c/MPoint/PyThOnS/kitploit.py |grep -v href |grep -v "/a" |grep -v "]"' 
...
alias MITM="cd /opt/MITMf/ && mitmf --arp --spoof --hsts --jskeylogger --ferretng --gateway"
...
alias NMStop="systemctl stop NetworkManager.service ; systemctl stop wpa_supplicant.service"
alias NMstop='systemctl stop NetworkManager ; systemctl stop wpa_supplicant.service'
...
alias NT='arp-scan -l -I $(ifconfig |grep flag|grep -v lo |cut -f1 -d":")'
alias NTNames="/usr/bin/arp-scan -l |sed 's/\/arp-scan\/)/\n/g' | sed 's/10.42.1.10\t/jaR0\t/g' | sed 's/10.42.1.100\t/RT\t/g' | sed 's/10.42.1.11\t/Landar0x\t/g' | sed 's/10.42.1.12\t/Knoppix\t/g' | sed 's/10.42.1.13\t/m4k\t/g' | sed 's/10.42.1.14\t/muS1x\t/g' | sed 's/10.42.1.15\t/linux-q5vi\t/g' | sed 's/10.42.1.16\t/R3d\t/g' | sed 's/10.42.1.17\t/xXx\t/g' | sed 's/10.42.1.18\t/f3dor\t/g' | sed 's/10.42.1.19\t/Matriux\t/g' | sed 's/10.42.1.20\t/BOX\t/g' | sed 's/10.42.1.21\t/tru30s\t/g' | sed 's/10.42.1.22\t/Spike\t/g' | sed 's/10.42.1.23\t/wifislax\t/g' | sed 's/10.42.1.25\t/w1ns3rv\t/g' | sed 's/10.42.1.27\t/bugtraq\t/g' | sed 's/10.42.1.28\t/n3th\t/g' | sed 's/10.42.1.29\t/pentoo\t/g' | sed 's/10.42.1.30\t/BOX\t/g' | sed 's/10.42.1.33\t/Ka0s\t/g' | sed 's/10.42.1.34\t/nSTool\t/g' | sed 's/10.42.1.31\t/fr33bsd\t/g' | sed 's/10.42.1.26\t/srv8e\t/g' | sed 's/10.42.1.24\t/LiGhT\t/g' | sed 's/10.42.1.32\t/parrot\t/g'| sed 's/10.42.1.35\t/minino\t/g'| sed 's/10.42.1.37\t/p3nt00\t/g'"
...
alias s='cd /pentest/PeNtEsT/ ; clear_console'
alias S="echo $(echo '$?')"
...
alias v='cd /var/www/html ; clear_console ; ls -ahl'
...
function MakePlayList { touch ./"$2" && find "$1" -print | egrep -ie '(*.mp3|*.wav|*.m4a|*.wma|*.ogg|*.mp4|*.mpc)'|sed 's/^/\"/g' |sed 's/$/\"/g'  >> "$2" ; echo ---DONE!--- ; }
...
alias ag='emerge --nospinner --noreplace --oneshot'
alias fadg='emerge -vt --ask --update --newuse --deep --with-bdeps=y --backtrack=200 --getbinpkg=y @world'
alias -g BT='--backtrack=200'
alias agSmooth='emerge --update --ask --autounmask -v1'
alias CLEAN='eclean-dist -i ; eclean-pkg; bleachbit -c --preset'
alias AS="qcheck"
