...
source /FILES/GITs/zsh-git-prompt/zshrc.sh
source /FILES/GITs/k/k.sh
...
if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.white)}%(#.💀.㉿) %*%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]%B%F{reset}%b$(git_super_status)\n%b%F{%(#.blue.green)}└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
    RPROMPT=$'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'

    # enable syntax-highlighting
    if [ -f /FILES/GITs/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh ] && [ "$color_prompt" = yes ]; then
	. /FILES/GITs/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh
...
if [ -f /FILES/GITs/zsh-autosuggestions/zsh-autosuggestions.zsh ]; then
    . /FILES/GITs/zsh-autosuggestions/zsh-autosuggestions.zsh
